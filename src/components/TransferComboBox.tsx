...
export default function TransferComboBox({
  selected,
  setSelected,
}: {
  selected: {
  ...
  const filteredTransfer =
    query === ""
      ...
      : transfers.filter((transfer) =>
          transfer.name.toLowerCase().includes(query.toLowerCase())
        );

  return (
    <>
      <Combobox value={selected} onChange={setSelected}>
        <Combobox.Label className={"justify-self-end"}>
          Transition Company
        </Combobox.Label>
        <div className="relative mt-1 w-[20rem] rounded-lg">
          ...
          <div className=" w-full border-[1px] rounded-md border-appGray/50 hover:ring-[3px] hover:ring-appGray/50 focus-within:border-appText">
            <Combobox.Input
              ...
              displayValue={(item: typeof transfers[0]) => item.name}
              onChange={(event) => setQuery(event.target.value)}
              placeholder="Select A Transition Service"
              required
            />
            <Combobox.Button
          ...
          >
            <Combobox.Options className="absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
              {filteredTransfer.length === 0 && query !== "" ? (
                <div className="relative cursor-default select-none py-2 px-4 text-gray-700">
                  Nothing found.
                </div>
              ) : (
                filteredTransfer.map((person) => (
                  <Combobox.Option
                    key={person.id}
                    className={({ active }) =>
